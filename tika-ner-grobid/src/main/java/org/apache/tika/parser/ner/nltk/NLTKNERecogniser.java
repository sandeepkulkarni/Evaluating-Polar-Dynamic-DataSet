...
public class NLTKNERecogniser implements NERecogniser {
    ...
    private static String readRestUrl() throws IOException {
        ...
        nltkProperties.load(NLTKNERecogniser.class.getClassLoader().getResourceAsStream("NLTKServer.properties"));
    ...
    public Map<String, Set<String>> recognise(String text) {
        ...
        try {
            ...
            Response response = WebClient.create(url).accept(MediaType.APPLICATION_JSON).post(text);
            int responseCode = response.getStatus();
            if (responseCode == 200) {
